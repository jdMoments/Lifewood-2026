...
const Employees: React.FC = () => {
  ...
  const employeeAdminTasks = adminTasks.filter(
    (task) => task.targetRole === 'employee' && (!task.assignedProfileId || task.assignedProfileId === user.id)
  );
  const filteredTasks = employeeAdminTasks.filter((task) =>
    (task.title || '').toLowerCase().includes(taskSearchTerm.trim().toLowerCase())
  );
  const completedTaskCount = employeeAdminTasks.filter((task) => Boolean(task.completed)).length;
  const totalTaskCount = employeeAdminTasks.length;
  const nowTimestamp = Date.now();
  const trackedProgressTasks = employeeAdminTasks.map((task) => {
            ...
            <div className="space-y-4">
              {filteredTasks.length > 0 ? (
                ...
                <div className={`p-4 rounded-2xl text-center text-sm ${darkMode ? 'bg-slate-900/40 text-slate-500 border border-slate-700' : 'bg-gray-50 text-gray-500 border border-gray-200'}`}>
                  {employeeAdminTasks.length === 0 ? 'No task from Admin yet.' : 'No task matched your search.'}
                </div>
              )}
            </div>
          </div>
          ...
          <section className="space-y-6">
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
              <div className={`xl:col-span-4 rounded-[32px] border p-6 shadow-sm ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-black/5'}`}>
                ...
                <h3 className={`mt-2 text-xl font-bold ${darkMode ? 'text-slate-100' : 'text-gray-900'}`}>My Work Track</h3>
